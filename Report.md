...
-   **Ask**: business understanding (identify the business task,
    consider key stakeholders)

-   **Prepare**: prepare data for analysis (store data, identify how
    it’s organized, sort and filter data, determine the credibility of
    the data)

-   **Process**: cleaning or manipulation of data (check the data for
    errors, transform the data)

-   **Analyze**: get the insights of data (organize and format your
    data, make some computations, identify trends and relationships)

-   **Share**: create visualizations and list key findings

-   **Act**: top three recommendations

R is used for preparation, cleaning and analysis. Tableau is for the
visualisation.
...
passes are categorized as **casual riders**. Customers who purchase
annual memberships are **Cyclistic members**. There are 692 stations
across Chicago. Rider can unlock from one station and return to any
...
The time frame is 12 months from *February 2022 to January 2023*. The
data is categorized into .csv files split up by each month. The
downloaded files are in .zip format. They needed to be unzipped into
.csv files before cleaning phase. There is one file that was named
...
(station-related variables are used to visualize locations).

    sum(is.na(df$start_station_id))

    ## [1] 514861

    sum(is.na(df$end_station_id)) 

    ## [1] 557950

After cleaning `start_station_name`, missing values drop from 843525 to
...
In Figure 1, average duration of casual riders are twice as much as that
of Cyclistic members. In the Figure 3, it shows that they bike more
around day 7 to 17 in a month and the trend goes down at the end of the
month.

![Figure 4. Top 5 locations](graphic/member_station.png)

As shown in Figure 4, end station locations are the same for member and
casual riders. But the start station locations of member customers are
different from that of casual bikers.

## CONCLUSION

After making some analyses from the dataset, here are some conclusions:

-   Even though casual customers have less rides, the duration of their
    rides are much higher than Cyclistic members.

-   The busiest time Casual riders use the Cyclistic services are in the
    weekend. In the opposite, members ride bikes during weekdays more.

-   During the summer, there is an increasing pattern for casual bikers
    especially for the electric bike.

-   In a month, day 7 to 17 have the highest duration for the casual
    customers. It drops down significantly at the end of the month.

-   There are different top 5 start station locations between casual and
    member riders. But they have the same top 5 end station locations.

## RECOMMENDATION

Based on what I have analyzed and concluded, the difference between
casual and member behaviour is more clear and help marketing team
approach more casual riders with their marketing campaign based on
data-driven decision making. I have some recommendations for the
marketing team:

-   Casual riders have higher average duration and bike more during the
    day 7 to 17 in a month: we should focus marketing campaign on the
    length of the rides to help casual riders see the benefits of
    converting to members instead of single-ride passes or full-day
    passes to save much money.

-   There is an increasing pattern of number of rides in the weekend for
    casual customers: a discount plan for weekend rides is only eligible
    for membership.

-   Seasonal time is the most busiest time: seasonal membership plan is
    necessary. The plan should espcially focus on the bike type:
    electric bike. Besides, docked bike is only used by casual
    customers. Marketing team can create a special plan only for docked
    bike.

-   At the top 5 start station locations, there will be location-related
    promotions. The conversion rate is higher with higher number of
    participants. Most of these locations are tourist areas. The
    collaboration with related-tourist business can help marketing
    campaign approach more tourists by offering better deal with cheaper
    price with the collaborators through the advantage of economy of
    scale.

# REFERENCES

[Github repo](https://github.com/lynnevo170701/case-study-1.git) for
more information (full R code, graphic…)  
[Figure
1](https://public.tableau.com/app/profile/linh7637/viz/Memberandcasualinsixdaysofaweek/mem_day_of_week)  
[Figure
2](https://public.tableau.com/app/profile/linh7637/viz/Memberandcasualusageinayear/Dashboard3)  
[Figure
3](https://public.tableau.com/app/profile/linh7637/viz/Averagedurationofmemberandcasualinamonth/Dashboard4)  
[Figure
4](https://public.tableau.com/app/profile/linh7637/viz/Top5stationlocation/member_location)
